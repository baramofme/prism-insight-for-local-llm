...
## 5. 실측 3자 비교 (GF · alpha · beta) — 2026-06-30

> **§4는 추정/구버전. 본 절이 Playwright 실측 기준 권위 데이터.**
> GF 베타(라이브)·alpha(`:3040` 인증 대시보드)·beta(`:3039` GF 클론)를 같은 폭에서 실측.
> resize가 스냅돼 실제 `innerWidth`를 매번 읽음. 영역은 상단 고정 헤더 + 키 큰 컬럼 geometric 검출.

### 5.1 폭별 실측 (대표값)

| 영역 | GF | alpha | beta |
|---|---|---|---|
| **헤더** | 풀폭 x0 (60/80px) | **x80~320 오프셋**(사이드바 뒤), 56px | **풀폭 x0**, 58px |
| **좌 네비 (~1100)** | 81 collapsed | 80 collapsed | 258 확장(내부스크롤) |
| **좌 네비 (~1500)** | 301~321 확장 | 300→320 확장 | 272 확장 |
| **우 패널 등장** | **자동 ~1040** | **없음(토글, 기본 닫힘 w0)** | **자동 ~1040** |
| **우 패널 폭** | 345→641 반응형 | — (고정 320, 열어야 보임) | 344→658 반응형 |
| **중앙 폭 (wide)** | 792 | 잔여(~1200) | 792 |
| **데스크톱 스크롤** | 앱셸(독립) | **페이지 통짜** | 앱셸(독립) |
| **모바일 가로스트립** | 2개 | 0 | 0 → **2 (2026-06-30 반영)** |

### 5.2 핵심 결론

- **beta ≈ GF 충실**: 풀폭 헤더·앱셸 독립 스크롤·우 패널 자동 등장(~1040, 344→658)·중앙 792 모두 GF와 일치.
- **alpha 구조적 이탈**: 헤더가 사이드바 뒤로 오프셋·우 패널은 토글(기본 닫힘, 자동 등장 X)·페이지 통짜 스크롤.
- **"GF 갭"의 실체는 alpha**: beta는 대부분 이미 GF 수준. alpha만 헤더 풀폭/우패널 자동등장/앱셸 스크롤에서 벗어남.

### 5.3 §4 대비 실측 정정 2건

1. **우 패널 등장점 = ~1040** (§4의 "1280"은 오류). GF·beta 모두 ~1040에서 자동 등장.
2. **GF 네비 호버 오버레이 실재**: 접힌 81px에 호버 → 321px 오버레이(fixed, z-index 11). §4.7의 "alpha 호버가 GF엔 없음"은 오류 — **alpha 호버 동작이 GF 정합**.

### 5.4 6갭 재판정 (실측)

| 갭 | GF | alpha | beta |
|---|---|---|---|
| 1. 우패널 자동등장·반응형폭 | 기준 | ❌ 토글 닫힘 | ✅ 정합 |
| 2. 독립(앱셸) 스크롤 | 기준 | ❌ 페이지 스크롤 | ✅ 정합 |
| 3. 네비 확장/호버 | 기준 | ✅ 정합(임계값만 미세차) | △ 항상 확장 |
| 4. 헤더 풀폭 | 기준 | ❌ 오프셋 | ✅ 풀폭 |
| 5. 중앙 792 | 기준 | △ 잔여폭 | ✅ 792 |
| 6. 모바일 가로스트립 | 2개 | ❌ 0 | ✅ **2 (반영 완료)** |

### 5.5 반영 내역 (beta)

- **#6 모바일 가로 스크롤 스트립** — `_components/overview/overview-content.tsx`:
  - 지수 스트립: 모바일에서 `flex overflow-x-auto` + 카드 `w-[104px]` (데스크톱은 5-col grid 유지)
  - 종목 테이블: 전 폭 가로 스크롤(모바일 세로 스택 제거), 모바일 테이블 `w-[300px]`
  - 실측: vw520에서 index-grid(552>416)·stock-tables(948>416) 둘 다 스크롤 → GF의 2개 스트립과 정합.

### 5.6 남은 alpha 정합 과제 (실측 확정)

1. 헤더 풀폭화 (현재 사이드바 뒤 오프셋)
2. 우 패널 자동 등장(~1040) + 반응형 폭 (현재 토글·기본 닫힘)
3. 데스크톱 앱셸 독립 스크롤 (현재 페이지 통짜)
4. 모바일 가로 스크롤 스트립 (beta와 동일 패턴 이식)
