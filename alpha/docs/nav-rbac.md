...
**Key Insight**: Navigation visibility is UX only, not security. We can check everything client-side using session-based hooks!
...
- **Session hooks provide all data client-side** - session state gives us permissions and role information
...
Plans and features require the `has()` function which is server-side only. Options:
