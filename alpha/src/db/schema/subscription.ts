// Stripe subscription state managed by @better-auth/stripe plugin.
// referenceId is polymorphic: user.id or organization.id.
import { boolean, index, integer, pgTable, text, timestamp } from "drizzle-orm/pg-core";
import { generateAuthId } from "./id";

export const subscription = pgTable("subscription", {
  id: text().primaryKey().$defaultFn(() => generateAuthId("subscription")),
  plan: text().notNull(),
