...
export function StockHeader({ quote }: { quote: Quote }) {
  ...
  return (
    <div className='space-y-1'>
      ...
      <h1 className='text-xl font-normal'>{quote.name}</h1>
      <div className='flex flex-wrap items-baseline gap-x-3 gap-y-1'>
        <span className='text-2xl font-medium tabular-nums'>{formatPrice(quote.price, quote.currency)}</span>
        <span className={`flex items-center gap-1 text-xs font-medium tabular-nums ${changeColorClass(quote.changePct)}`}>
